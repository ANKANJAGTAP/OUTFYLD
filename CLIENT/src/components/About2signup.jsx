...
const SignupForm = () => {
  ...
  useEffect(() => {
    if (buttonRef.current && iconRef.current && !isSubmitting) {
      ...
      const onEnter = () =>
        gsap.to(icon, { rotation: 360, scale: 1.2, duration: 0.5, ease: 'back.out' });
      const onLeave = () =>
        gsap.to(icon, { rotation: 0, scale: 1, duration: 0.5, ease: 'back.out' });
        ...
        {isSubmitted ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-6" noValidate>
            <div>
              <label
                htmlFor="name"
                className="block mb-2 font-medium text-charcoal dark:text-white"
              >
                Full Name
              </label>
              <input
